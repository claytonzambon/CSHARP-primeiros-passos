...
namespace _69._7._URI_1187 {
    class Program {
        static void Main(string[] args) {
            ...
            for (int i = 0; i < 12; i++) {
                for (int j = 0; j < 12; j++) {
                    //Se a soma de  "i < j" e "i + j < 11" (tamanho máximo da linha da matriz)
                    //realizamos a operação selecionada apenas com a Area Superior da Matriz
                    if (i < j && (i + j) < 11) {
                        if (O == 'S') {
                            resultado += M[i, j];
                        }
